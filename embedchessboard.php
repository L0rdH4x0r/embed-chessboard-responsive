...
Copyright: copyright (C) 2009-2016 Paolo Casaschi
...
class pgnBBCode {
  ...
  function shortcode_pgn( $atts = array(), $content = NULL ) {
    ...
    // Responsive
    $replacement .= "<script>";
    $replacement .= "window.matchMedia=window.matchMedia||function(a,b){var c,d=a.documentElement,e=d.firstElementChild||d.firstChild,f=a.createElement(\"body\"),g=a.createElement(\"div\");return g.id=\"mq-test-1\",g.style.cssText=\"position:absolute;top:-100em\",f.style.background=\"none\",f.appendChild(g),function(a){return g.innerHTML='&shy;<style media=\"'+a+'\"> #mq-test-1 { width: 42px; }</style>',d.insertBefore(f,e),c=42===g.offsetWidth,d.removeChild(f),{matches:c,media:a}}}(document);";
    $replacement .= "(function(){";
    
    $replacement .= "var responsiveFix = function (mq) {";
    $replacement .= "    var ifr = document.getElementById('".$pgnId."').parentNode.getElementsByTagName('iframe')[0];";
    $replacement .= "    if (!ifr) return;";
    $replacement .= "    if (mq.matches) {";
    $replacement .= "        ifr.height=".(2*$height).";";
    $replacement .= "    } else ifr.height=".($height).";";
    $replacement .= "};";
    $replacement .= "if (matchMedia) {";
    $replacement .= "  var mq = window.matchMedia(\"(max-width:30em)\");";
    $replacement .= "  mq.addListener(responsiveFix);";
    $replacement .= "  responsiveFix(mq);";
    $replacement .= "}";
